...
RLAPI void rlEnableDither(void);                   		// Enable dithering
RLAPI void rlDisableDither(void);                  		// Disable dithering
...
unsigned int rlLoadTexture(const void *data, int width, int height, int format, int mipmapCount)
{
    ...
    for (int i = 0; i < mipmapCount; i++)
    {
        ...
        {
            if (format < RL_PIXELFORMAT_COMPRESSED_DXT1_RGB) glTexImage2D(GL_TEXTURE_2D, i, glInternalFormat, mipWidth, mipHeight, 0, glFormat, glType, data != NULL ? (unsigned char *)data + mipOffset : 0);
#if !defined(GRAPHICS_API_OPENGL_11)
            else glCompressedTexImage2D(GL_TEXTURE_2D, i, glInternalFormat, mipWidth, mipHeight, 0, mipSize, data != NULL ? (unsigned char *)data + mipOffset : 0);
#endif
