...
void DrawTexturePro(Texture2D texture, Rectangle source, Rectangle dest, Vector2 origin, float rotation, Color tint)
{
    ...
    if (texture.id > 0)
    {
		...
		//			left = 		(2 * source.x + 0.125f) / (2 * width);
		//			right = 	(2 * source.x + source.width * 2 - 0.125f) / (2 * width);;
		//			top = 		(2 * source.y + 0.125f) / (2 * height);
		//			bottom = 	(2 * source.y + source.height * 2 - 0.125f) / (2 * height);

		
//			int stretchingFactor = 2;
//			float stretchingAmount = 0.25;
			int stretchingFactor = 1;
			float stretchingAmount = 0.001f;

			left = 		(stretchingFactor * source.x) / (stretchingFactor * width);
			right = 	(stretchingFactor * source.x + source.width * stretchingFactor - stretchingAmount) / (stretchingFactor * width);;
			top = 		(stretchingFactor * source.y) / (stretchingFactor * height);
			bottom = 	(stretchingFactor * source.y + source.height * stretchingFactor - stretchingAmount) / (stretchingFactor * height);
