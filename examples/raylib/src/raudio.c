...
Music DuplicateMusicStream(Music* music){
	Music duplicate = { 0 };

	drmp3* pMP3 = (drmp3*)music->ctxData;
	...
	drmp3_uint8* data = calloc( 1, lSize + 1 );

	fread(data , lSize, 1 , pFile);
	
	data[lSize] = 0;
		
	int result = drmp3_init_memory(ctxMp3, data, lSize + 1, NULL);
	
	//ctxMp3->pUserData = pMP3->pUserData;
	duplicate.ctxType = MUSIC_AUDIO_MP3;
	duplicate.ctxData = ctxMp3;

	if (result > 0)
	{
		duplicate.stream = LoadAudioStream(pMP3->sampleRate, 32, pMP3->channels);
		duplicate.frameCount = (unsigned int)drmp3_get_pcm_frame_count(pMP3);
		duplicate.looping = true;   // Looping enabled by default
	}
